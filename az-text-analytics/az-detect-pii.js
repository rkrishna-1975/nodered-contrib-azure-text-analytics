...
module.exports = function (RED) {
    function AZDetectPII(config) {
        RED.nodes.createNode(this, config);
        var node = this;
        node.debug("Loaded the pii detect node");
        ...
        async function detectEntity(client, items, node, msg, done) {
            try {
                const entityResult = await client.recognizePiiEntities(items);
    ...
    RED.nodes.registerType("az-detect-pii", AZDetectPII);
}